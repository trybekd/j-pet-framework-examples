...
JPetEvent EventCategorizer::physicsAnalysis( vector<JPetHit> hits )
{
	...
	if( deexcitationHits.getHits().size() == 1 )
	{
		physicEvent.setEventType(JPetEventType::kPrompt);
		if( annihilationHits.getHits().size() > 1 )
		{
		  auto ahits = annihilationHits.getHits(); 
		  for(auto &hit : ahits)
			getStatistics().getHisto1D("Positronium_Lifetime")->Fill( hit.getTime()/1000. - deexcitationHits.getHits().at(0).getTime()/1000. );
		}
	}
