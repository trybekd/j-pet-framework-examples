 ...
 */
const map<int, vector<JPetSigCh>> SignalFinderTools::getSigChByPM(const JPetTimeWindow* timeWindow, bool useCorrupts, int refPMID)
{
  ...
  for (unsigned int i = 0; i < nSigChs; i++)
  {
    ...
    if (!useCorrupts && sigCh.getRecoFlag() == JPetSigCh::Corrupted)
    {
      continue;
    }
...
vector<JPetRawSignal> SignalFinderTools::buildAllSignals(const map<int, vector<JPetSigCh>>& sigChByPM, double sigChEdgeMaxTime,
                                                         double sigChLeadTrailMaxTime, JPetStatistics& stats, bool saveHistos,
                                                         ThresholdOrderings thresholdOrderings)
{
  ...
  for (auto& sigChPair : sigChByPM)
  {
    ...
    auto signals = buildRawSignals(sigChPair.second, sigChEdgeMaxTime, sigChLeadTrailMaxTime, stats, saveHistos, P);
...
vector<JPetRawSignal> SignalFinderTools::buildRawSignals(const vector<JPetSigCh>& sigChByPM, double sigChEdgeMaxTime, double sigChLeadTrailMaxTime,
                                                         JPetStatistics& stats, bool saveHistos, Permutation ordering)
{
  ...
  while (thrLeadingSigCh.at(0).size() > 0)
  {
    ...
    {
      int nextThrSigChIndex = findSigChOnNextThr(thrLeadingSigCh.at(0).at(0).getValue(), sigChEdgeMaxTime, thrLeadingSigCh.at(kk));
      if (nextThrSigChIndex != -1)
      {
        closestTrailingSigCh = findTrailingSigCh(thrLeadingSigCh.at(0).at(0), sigChLeadTrailMaxTime, thrTrailingSigCh.at(kk));
        if (closestTrailingSigCh != -1)
        {
...
void SignalFinderTools::permuteThresholdsByValue(const ThresholdValues& threshold_values, Permutation& new_ordering)
{
  ...
  sort(indices.begin(), indices.end(), [&](const int& a, const int& b) { return (threshold_values.at(a) < threshold_values.at(b)); });
