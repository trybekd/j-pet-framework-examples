...
HitFinderTools::TOTCalculationType HitFinderTools::getTOTCalculationType(const string& type)
{
  if (type == "rectangular")
  {
...
/**
 * Calculation of the total TOT of the hit - sum of sigA and sigB TOTs.
 */
double HitFinderTools::calculateTOT(const JPetHit& hit, TOTCalculationType type)
{
  return calculateSignalTOT(hit.getSignalA(), type) + calculateSignalTOT(hit.getSignalB(), type);
}

/**
 * Calculation of the total TOT of the signal - Time over Threshold:
 * the weighted sum of the TOTs on all of the thresholds (1-4) and on the both sides (A,B)
 */
double HitFinderTools::calculateSignalTOT(const JPetPhysSignal& signal, TOTCalculationType type)
{
  double tot = 0.0, weight = 1.0;
  auto totTHRMap = signal.getRecoSignal().getRawSignal().getTOTsVsThresholdValue();

  if (!totTHRMap.empty())
  {
    double firstThr = totTHRMap.begin()->first;
    tot += weight * totTHRMap.begin()->second;
    if (totTHRMap.size() > 1)
    {
      for (auto it = next(totTHRMap.begin(), 1); it != totTHRMap.end(); it++)
      {
        switch (type)
        {
        ...
        case TOTCalculationType::kThresholdRectangular:
          weight = (it->first - prev(it, 1)->first) / firstThr;
          break;
        case TOTCalculationType::kThresholdTrapeze:
          weight = (it->first - prev(it, 1)->first) / firstThr;
          tot += weight * (it->second - prev(it, 1)->second) / 2;
          break;
        }
