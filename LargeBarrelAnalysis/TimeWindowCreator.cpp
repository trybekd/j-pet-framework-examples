...
bool TimeWindowCreator::init()
{
  ...
  auto calibFile = string("dummyCalibration.txt");
...
bool TimeWindowCreator::exec()
{
  if (auto event = dynamic_cast<EventIII* const>(fEvent))
  {
    ...
    if (fSaveControlHistos)
    {
      getStatistics().fillHistogram("sigch_tslot", kTDCChannels);
    }
...
{
  getStatistics().createHistogramWithAxes(new TH1D("sigch_tslot", "Signal Channels Per Time Slot", 250, -0.125, 999.875),
